...
function Notification(options = {}) {
  ...
  const dialogButtons = () => {
    return `<div class="notification-buttons">
      <button class="notification-button notification-cancel">Cancel</button>
      <button class="notification-button notification-action">Confirm</button>
    </div>`;
  };
  ...
  const templatePopup = (type) => {
    const closeEl = `<button class="notification-close" aria-label="Close">×</button>`;
    const iconEl = `<div class="notification-icon">${dataByType[type].icon}</div>`;
    const titleBlock = `<div class="notification-title">
      ${iconEl}
      <div class="title"></div>
      ${closeEl}
    </div>`;
  ...
  const createPopup = (type) => {
    ...
    elPopup.insertAdjacentHTML('beforeend', templatePopup(type));
